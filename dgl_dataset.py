...
class NASBench101CellDataset(DGLDataset):
    def __init__(self, num_arch, generate_data=True):
    ...
    def save(self):
        ...
        graph_path = os.path.join(self.save_dir + '_dgl_graph.bin')
        save_graphs(graph_path, list(self.graphs), {'fitness': torch.Tensor(np.array(self.labels))})
        info_dict = {'dim_nfeats': self.dim_nfeats}
        save_info(os.path.join(self.save_dir + '_dgl_graph_info.pkl'), info_dict)
    ...
    def load(self):
        ...
        info_dict = load_info(os.path.join(self.save_dir + '_dgl_graph_info.pkl'))
