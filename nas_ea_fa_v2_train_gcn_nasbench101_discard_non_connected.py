...
from nasbench101_utils_dnc import Architecture
from nasbench101_utils_dnc import randomly_sample_architecture, create_nord_architecture, build_connection_matrix, \
...
def NAS_EA_FA_V2_train_gcn_101():
    # Instantiate the evaluator
    evaluator = BenchmarkEvaluator()

    dir_name_results = 'results_nas_ea_fa_v2_dnc101_train_gcn_{}epochs_trainperc{}_K{}_H{}_populationsize{}_unique'.format(NUM_EPOCHS,
                                                                                                                            int(TRAIN_PERCENTAGE * 100),
                                                                                                                            K, H, POPULATION_SIZE)
    dir_name_dgl_train_stats = 'nas_ea_fa_v2_dnc101_dgl_model_train_stats_{}epochs_trainperc{}_K{}_H{}_populationsize{}_unique'.format(NUM_EPOCHS,
                                                                                                                                        int(TRAIN_PERCENTAGE * 100),
                                                                                                                                        K, H, POPULATION_SIZE)
    dir_name_dgl_eval_stats = 'nas_ea_fa_v2_dnc101_dgl_model_eval_stats_{}epochs_trainperc{}_K{}_H{}_populationsize{}_unique'.format(NUM_EPOCHS,
                                                                                                                                      int(TRAIN_PERCENTAGE * 100),
                                                                                                                                      K, H, POPULATION_SIZE)
    ...
    for exp_repeat_index in range(EXP_REPEAT_TIMES):
        ...
        while t < T:
            ...
            for arch_index in range(len(population)):
                ...
                # # check whether architecture is already in new population
                # architecture_exists = False
                # if new_population != []:
                #     for arch in new_population:
                #         if architecture.layers == arch.layers and np.array_equal(architecture.connections, arch.connections) and architecture.fitness == arch.fitness:
                ...
                G = d.to_networkx(benchmark_dataset='nasbench101')
                # convert networkx to dgl object
                architecture.dgl_obj = dgl.from_networkx(G, node_attrs=['layer'])

                # add architecture to training set
                if architecture.connections.tolist() not in x_train:
                    x_train.append(architecture.connections.tolist())
                    x_train_graphs.append(architecture.dgl_obj)
                    y_train.append(architecture.fitness)
                ...
                for sequence in isomorphic_sequences:
                    a = Architecture(architecture.layers, np.array(sequence, dtype=int), build_connection_matrix(sequence),
            ...
            while num_arch < K + H and current_time_budget <= MAX_TIME_BUDGET:
                ...
                # if new_population != []:
                #     for arch in new_population:
                #         if architecture.layers == arch.layers and np.array_equal(architecture.connections,
                #                                                                  arch.connections) \
                #                 and architecture.fitness == arch.fitness:
                #             architecture_exists = True
                #             break
                # if architecture_exists:
                #     continue
            ...
            model_handler.plot_losses(stats_save_dir=os.path.join(dir_name_dgl_train_stats, 'experiment' + str(exp_repeat_index + 1)),
                                      model_name='iteration' + str(t))
            ...
            model_handler.evaluate(stats_save_dir=os.path.join(dir_name_dgl_eval_stats, 'experiment' + str(exp_repeat_index + 1)),
                                   model_name='iteration' + str(t))
