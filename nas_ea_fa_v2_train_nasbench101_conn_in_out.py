...
from nasbench101_utils_cio import EXP_REPEAT_TIMES, MAX_TIME_BUDGET, POPULATION_SIZE, MAX_CONNECTIONS, NUM_GEN, K, H
from nasbench101_utils_cio import Architecture
from nasbench101_utils_cio import randomly_sample_architecture, build_connection_matrix, create_nord_architecture, \
...
def NAS_EA_FA_V2():
    ...
    if not os.path.exists('results_nas_ea_fa_v2_cio101_train'):
        os.mkdir('results_nas_ea_fa_v2_cio101_train')
    for exp_repeat_index in range(EXP_REPEAT_TIMES):
        start_time = time.time()
        folder_name = 'results_nas_ea_fa_v2_cio101_train/results' + str(exp_repeat_index + 1)
        ...
        for _ in range(POPULATION_SIZE):
            ...
            while not is_valid_architecture:
                layers, connections, fitness = randomly_sample_architecture()
                # check if connection number is ok for nasbench-101
                if sum(connections) <= MAX_CONNECTIONS:
                    total_population.append(
                        Architecture(layers, connections, build_connection_matrix(connections), fitness))
        ...
        while current_time_budget <= MAX_TIME_BUDGET:
            ...
            for arch_index in range(len(population)):
                ...
                for _ in range(len(isomorphic_sequences)):
                    y_train.append(val_acc)
                # # print('isomorphic sequences initial length:', len(isomorphic_sequences))
                ...
                # # print('unique isomorphic sequences length:', len(isomorphic_sequences_unique))
            ...
            while num_arch < K + H and current_time_budget <= MAX_TIME_BUDGET:
                ...
                #   if not any(np.array_equal(arr, unique_arr) for unique_arr in isomorphic_sequences_unique):
                #     isomorphic_sequences_unique.append(arr)
                # # print('isomorphic sequences length:', len(isomorphic_sequences_unique))
                # x_train.extend(isomorphic_sequences_unique)
                # for _ in range(len(isomorphic_sequences_unique)):
                #     y_train.append(val_acc)
